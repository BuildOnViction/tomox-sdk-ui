//@flow
import type {
  ConvertTokensFormEvent,
  ConvertTokensFormState
} from '../../types/convertTokensForm';
...
export const convertTxReverted = (
  tokenSymbol: string,
  receipt: TxReceipt
): ConvertTokensFormEvent => {
  const event = (state: ConvertTokensFormState) => ({
...
export default function convertTokensFormDomain(state: ConvertTokensFormState) {
  return {
    ...
    convertTxState(tokenSymbol: string) {
      return {
        convertTxStatus:
          state[tokenSymbol] && state[tokenSymbol].convertTxStatus,
        convertTxHash: state[tokenSymbol] && state[tokenSymbol].convertTxHash,
        convertTxReceipt:
          state[tokenSymbol] && state[tokenSymbol].convertTxReceipt
      };
    }
  };
}

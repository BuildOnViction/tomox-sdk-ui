...
describe('wallet service', () => {
  ...
  const encrypt = jest.fn(() => Promise.resolve('test serialized wallet'))
  ethers.Wallet.createRandom = jest.fn(() => Promise.resolve({ address, encrypt }))

  it('createAndEncryptWallet should work correctly', async () => {
    const result = await walletService.createAndEncryptWallet(password, callback)
...
describe('wallet service (integration with ethers', () => {
  it('createWalletFromJSON should return correct json file', async () => {
    ethers.Wallet.fromEncryptedWallet.mockImplementationOnce(require.requireActual('ethers').Wallet.fromEncryptedWallet)
    const json =
    ...
    const { wallet, encryptedWallet } = await walletService.createWalletFromJSON(json, 'coolcoolcool')
  ...
  it('createRandomWallet should return a json file with an address and a private key', async () => {
    ...
    const wallet = await walletService.createRandomWallet()
  ...
  it.skip('should encrypt a wallet with a password', async () => {
    ...
    const { address, encryptedWallet } = await walletService.createAndEncryptWallet('coolcoolcoolcool', callback)
  ...
  it.skip('should encrypt and decrypt a wallet with a password', async () => {
    ...
    const decryptedWallet = await walletService.decryptWallet(encryptedWallet, 'coolcoolcoolcool')

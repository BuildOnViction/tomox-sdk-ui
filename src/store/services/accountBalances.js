...
export async function queryTokenBalances(address: string, tokens: Array<Token>): TokenBalances {
  let balances
  ...
  balances = (balances: TokenBalances).filter(balance => balance !== null).map((balance, i) => ({
...
export async function queryExchangeTokenAllowances(owner: string, tokens: Array<Token>) {
  ...
  allowances = (allowances: TokenBalances).filter(allowance => allowance !== null).map((allowance, i) => ({
    symbol: tokens[i].symbol,

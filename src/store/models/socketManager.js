  ...
  return (dispatch, getState, { socket }) => {
    ...
    const closeConnection = socket.openConnection(event => {
      switch (event.type) {
        ...
        case 'open':
          return dispatch(
            appActionCreators.addSuccessNotification({ message: 'Reconnected' })
          );
        default:
          break;
      }
...
function handleOrderAdded(event: WebsocketEvent): ThunkAction {
  return async (dispatch, getState, { socket }) => {
    try {
      ...
      dispatch(
        appActionCreators.addSuccessNotification({ message: 'Order added' })
      );
...
function handleOrderCancelled(event: WebsocketEvent): ThunkAction {
  return async (dispatch, getState, { socket }) => {
    try {
      ...
      dispatch(
        appActionCreators.addSuccessNotification({ message: 'Order cancelled' })
      );
      dispatch(actionCreators.updateOrdersTable([order]));
    } catch (e) {
      console.log(e);
...
function handleOrderSuccess(event: WebsocketEvent): ThunkAction {
  return async (dispatch, getState, { socket }) => {
    try {
      ...
      let orders = matches
        .filter(match => match.order.userAddress === signerAddress)
        .map(match => parseOrder(match.order));

      dispatch(
        appActionCreators.addSuccessNotification({ message: 'Order success' })
      );
...
function handleOrderPending(event: WebsocketEvent): ThunkAction {
  return async (dispatch, getState, { socket }) => {
    try {
      ...
      dispatch(actionCreators.updateOrdersTable(orders));
      dispatch(
        appActionCreators.addSuccessNotification({ message: 'Order pending' })
      );
    } catch (e) {
      console.log(e);
      dispatch(appActionCreators.addErrorNotification({ message: e.message }));
...
function handleRequestSignature(event: WebsocketEvent): ThunkAction {
  return async (dispatch, getState, { socket }) => {
    try {
      ...
      dispatch(
        appActionCreators.addSuccessNotification({ message: 'Signing trade' })
      );
      socket.sendNewSubmitSignatureMessage(
        hash,
        order,
        remainingOrder,
        matches
